...
namespace HashTag.ApplicationServices.Wcf
{
  /// <summary>
  ...
  /// </summary>
    public class WcfMembershipProvider : MembershipProvider, IDisposable, IMembershipProvider
    {
        ...
        IWcfApplicationService _membershipService;
        ...
        public WcfMembershipProvider(string serviceUri, string clientEndPointName = "HashTagApplicationServiceEndpoint")
        {
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            //{
            //    ExceptionFactory.Throw<ConfigurationErrorsException>("Required attribute 'serviceUri' missing from provider configuration. Value may contain HashTag configuration tokens: {app}, {env}, {user}, {machine}, {%app:<key>%}, and {%connection:<key>%}");
            //}

            var endpointName = _config["clientEndPointName"];
            if (config.AllKeys.Contains("clientEndPointName") == false || string.IsNullOrEmpty(config["clientEndPointName"]) == true)
            {
                ExceptionFactory.Throw<ConfigurationErrorsException>("Required attribute 'clientEndPointName' missing from provider configuration. Value may contain HashTag configuration tokens: {app}, {env}, {user}, {machine}, {%app:<key>%}, and {%connection:<key>%}");
            }

            createService(svcUri,endpointName);
        ...
        public override MembershipUser CreateUser(string username, string password, string email, string passwordQuestion, string passwordAnswer, bool isApproved, object providerUserKey, out MembershipCreateStatus status)
        {
            //using (_log.NewTrace())
            {
                ...
                try
                {
                    ...
                    request.User = new MembershipUser(this.Name, username, providerUserKey, email, passwordQuestion, "", isApproved, false, DateTime.UtcNow, Utils.Date.SqlMinDate, Utils.Date.SqlMinDate, DateTime.UtcNow, Utils.Date.SqlMinDate);
                    request.Password = password;
        ...
        public override bool ValidateUser(string username, string password)
        {
            //using (_log.NewTrace())
            {
                ...
                try
                {
                    ...
                    _log.Warning.Write("User '{0}' is not validated with status '{1}'. {2}",
